        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <form class="row g-3" method="POST" action="{{ route('author.update', $profile->id)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="col-6">
                            <label for="inputNanme4" class="form-label">Masukan Nama Author</label>
                            <input type="text" class="form-control" name="nama" value="{{ old('nama', $profile->nama) }}">
                            @error('nama')
