                ...
                <div class="card mb-4">
                    <div class="card-header pb-0">
                        ...
                        <a href="{{ route('author.create') }}" class="btn btn-sm btn-primary shadow-sm">
                            Tambah</a>
                        <a href="{{ url('profile-pdf') }}" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm" title="Export To PDF">
                                    ...
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <form method="POST" action="{{ route('author.destroy', $row->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                <a href="{{ route('author.edit', $row->id) }}" class="btn btn-info shadow-sm text-xs" data-toggle="tooltip" data-original-title="Edit user">
                                                    Edit
                                                </a>
                                                &nbsp;
                                                <button type="submit" class="btn btn-danger shadow-sm text-xs" onclick="return confirm('Anda yakin ingin hapus author ini?')" data-toggle="tooltip" data-original-title="Edit user">
                                                    Hapus
                                                </button>
                                            </form>
                                        </td>
